...
* For dashboard page/feature expansion, first read `app/dashboard/team/actions.tsx` and `app/dashboard/team/page.tsx` as baseline examples before proposing or generating new dashboard features; if either file is missing, state that explicitly and continue with the closest existing dashboard equivalent
...
* Emit code in deterministic dependency order to reduce dev-server breakage:
  ...
  3) migration generation/update notes
  4) server actions/routes/business logic
  5) reusable UI primitives/components
  6) route-level pages/layout integration
  7) validations/tests
...
* When implementing dashboard features, follow this strict hierarchy:
  ...
  2) include migration artifacts and call out `npm run db:generate` then `npm run db:migrate` with action tag [run]npm run db:generate || npm run db:migrate[/run]
  ...
  5) wire navigation/layout if the feature is new
